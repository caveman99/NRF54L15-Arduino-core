...
public:
    ...
    bool concat(unsigned char value, unsigned char base = 10)
    {
        String tmp(value, base);
        return concat(tmp);
    }

    bool concat(int value, unsigned char base = 10)
    {
        String tmp(value, base);
        return concat(tmp);
    }

    bool concat(unsigned int value, unsigned char base = 10)
    {
        String tmp(value, base);
        return concat(tmp);
    }

    bool concat(long value, unsigned char base = 10)
    {
        String tmp(value, base);
        return concat(tmp);
    }

    bool concat(unsigned long value, unsigned char base = 10)
    {
        String tmp(value, base);
        return concat(tmp);
    }
...
private:
    ...
    void assignUnsigned(unsigned long value, unsigned char base)
    {
        ...
        *ptr = '\0';
        do {
            ...
            *--ptr = (digit < 10U) ? (char)('0' + digit) : (char)('A' + (digit - 10U));
            value /= base;
        } while (value > 0U);
        assign(ptr);
    }
    ...
    void assignSigned(long value, unsigned char base)
    {
        if (base == 10U && value < 0) {
            char tmp[34];
            tmp[0] = '-';
            String rest((unsigned long)(-value), base);
            strncpy(&tmp[1], rest.c_str(), sizeof(tmp) - 2U);
            tmp[sizeof(tmp) - 1] = '\0';
            assign(tmp);
            return;
        }
