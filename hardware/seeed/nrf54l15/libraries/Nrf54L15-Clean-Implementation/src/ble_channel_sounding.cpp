...
constexpr uint8_t kMaxAuxDataLanes = 2U;
constexpr uint8_t kAuxDataWordsPerLane = 2U;
constexpr uint8_t kAuxDataLaneCount =
    static_cast<uint8_t>(((RADIO_AUXDATADMA_MaxCount < RADIO_AUXDATA_CNF_MaxCount)
                              ? RADIO_AUXDATADMA_MaxCount
                              : RADIO_AUXDATA_CNF_MaxCount) > kMaxAuxDataLanes
                             ? kMaxAuxDataLanes
                             : ((RADIO_AUXDATADMA_MaxCount < RADIO_AUXDATA_CNF_MaxCount)
                                    ? RADIO_AUXDATADMA_MaxCount
                                    : RADIO_AUXDATA_CNF_MaxCount));
constexpr size_t kMaxSlopePairs =
    (kMaxCsChannels * (kMaxCsChannels - 1U)) / 2U;

static_assert(kAuxDataLaneCount >= 1U, "RADIO AUXDATA support is required");
static_assert(kAuxDataLaneCount <= kMaxAuxDataLanes,
              "Unexpected RADIO AUXDATA lane count");
...
uint32_t makeAuxDataCaptureConfig() {
  uint32_t config =
      ((RADIO_AUXDATA_CNF_ACQMODE_Rtt << RADIO_AUXDATA_CNF_ACQMODE_Pos) &
       RADIO_AUXDATA_CNF_ACQMODE_Msk);
#if defined(RADIO_AUXDATA_CNF_DIR_Acq) && defined(RADIO_AUXDATA_CNF_DIR_Pos) && \
    defined(RADIO_AUXDATA_CNF_DIR_Msk)
  config |= ((RADIO_AUXDATA_CNF_DIR_Acq << RADIO_AUXDATA_CNF_DIR_Pos) &
...
bool BleChannelSoundingRadio::configureBle2MCommon() {
  ...
  for (uint8_t i = 0U; i < kAuxDataLaneCount; ++i) {
    radio_->AUXDATADMA[i].PTR =
        static_cast<uint32_t>(reinterpret_cast<uintptr_t>(
            &auxDataWords_[i * kAuxDataWordsPerLane])) &
        RADIO_AUXDATADMA_PTR_PTR_Msk;
    radio_->AUXDATADMA[i].MAXCNT =
        (static_cast<uint32_t>(kAuxDataWordsPerLane)
         << RADIO_AUXDATADMA_MAXCNT_MAXCNT_Pos) &
...
void BleChannelSoundingRadio::prepareAuxDataCapture() {
  ...
  for (uint8_t i = 0U; i < kAuxDataLaneCount; ++i) {
    radio_->AUXDATA.CNF[i] = makeAuxDataCaptureConfig();
    radio_->AUXDATADMA[i].PTR =
        static_cast<uint32_t>(reinterpret_cast<uintptr_t>(
            &auxDataWords_[i * kAuxDataWordsPerLane])) &
        RADIO_AUXDATADMA_PTR_PTR_Msk;
...
void BleChannelSoundingRadio::captureAuxDataRtt(BleCsRttSample* outRtt) {
  ...
  BleCsRttSample candidates[kMaxAuxDataLanes] = {};
  for (uint8_t i = 0U; i < kAuxDataLaneCount; ++i) {
    ...
    memcpy(candidates[i].rawBytes, &auxDataWords_[i * kAuxDataWordsPerLane],
           byteCount);
    parseRttRaw(&candidates[i]);
  }

  uint8_t selected = 0U;
  if (kAuxDataLaneCount > 1U && candidates[1].present) {
