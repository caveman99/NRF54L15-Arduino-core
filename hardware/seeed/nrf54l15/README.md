...
- `Nrf54L15-Clean-Implementation` now includes the clean-core board, BLE advertiser, BLE scanner, BLE connection, BLE custom GATT/NUS, low-power, diagnostics, and next peripherals parity tranches. Verified sketches in the current parity passes include `AarResolvePrivateAddress`, `BleActiveScanner`, `BleAdvertiser`, `BleBatteryNotifyPeripheral`, `BleConnectSerialMonitor`, `BleConnectionPeripheral`, `BleCustomGattRuntime`, `BleNordicUartCommandConsole`, `BlePassiveScanner`, `CcmBlePacketTamperDetect`, `CcmBleSpecVector`, `CompThresholdMonitor`, `CracenRandomBytes`, `CracenSeedArduinoRandom`, `DppicHardwareBlink`, `EcbAesKnownVector`, `GrtcCompareAlarmTicker`, `GrtcUptimeClock`, `I2sDuplexWrapperInterrupt`, `I2sTxWrapperInterrupt`, `LowPowerBleBeaconDutyCycle`, `LpcompSystemOffWake`, `QdecRotaryReporter`, `RawI2sTxInterrupt`, `SaadcDifferentialProbe`, and `SpisTargetEcho`.
- The current Zephyr-safe clean shims cover SPI/TWI/UART/ADC/timer/PWM/GPIOTE/PDM/I2S/QDEC/SPIS plus software-backed RNG/AAR/ECB/CCM, raw advertiser-compatible BLE shims (custom static address, raw AD/scan-response payloads, one-shot legacy advertising), BLE passive/active scanner shims, host-backed BLE connection shims (`isConnected`, `getConnectionInfo`, `pollConnectionEvent`, `isConnectionEncrypted`), clean-style custom GATT/NUS services, and the `Grtc` compatibility layer used by the new peripheral examples.
- The packaged Zephyr base config now sets `CONFIG_BT_ID_MAX=2` for custom clean-core advertiser identities and `CONFIG_BT_BAS=y` so clean battery/GATT examples expose a real Battery Service.
