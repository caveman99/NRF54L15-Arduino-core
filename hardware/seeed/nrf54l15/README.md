...
- `Nrf54L15-Clean-Implementation` now includes the clean-core board, BLE advertiser, BLE scanner, BLE connection, BLE custom GATT/NUS, BLE pairing/bond persistence, BLE channel sounding, low-power, diagnostics, and next peripherals parity tranches. Verified sketches in the current parity passes include `AarResolvePrivateAddress`, `BleActiveScanner`, `BleAdvertiser`, `BleBatteryNotifyPeripheral`, `BleBondPersistenceProbe`, `BleChannelSoundingInitiator`, `BleChannelSoundingReflector`, `BleConnectSerialMonitor`, `BleConnectionPeripheral`, `BleCustomGattRuntime`, `BleNordicUartCommandConsole`, `BlePairingEncryptionStatus`, `BlePassiveScanner`, `CcmBlePacketTamperDetect`, `CcmBleSpecVector`, `CompThresholdMonitor`, `CracenRandomBytes`, `CracenSeedArduinoRandom`, `DppicHardwareBlink`, `EcbAesKnownVector`, `GrtcCompareAlarmTicker`, `GrtcUptimeClock`, `I2sDuplexWrapperInterrupt`, `I2sTxWrapperInterrupt`, `LowPowerBleBeaconDutyCycle`, `LpcompSystemOffWake`, `QdecRotaryReporter`, `RawI2sTxInterrupt`, `SaadcDifferentialProbe`, and `SpisTargetEcho`.
- The current Zephyr-safe clean shims cover SPI/TWI/UART/ADC/timer/PWM/GPIOTE/PDM/I2S/QDEC/SPIS plus software-backed RNG/AAR/ECB/CCM, raw advertiser-compatible BLE shims (custom static address, raw AD/scan-response payloads, one-shot legacy advertising), BLE passive/active scanner shims, host-backed BLE connection shims (`isConnected`, `getConnectionInfo`, `pollConnectionEvent`, `isConnectionEncrypted`), clean-style custom GATT/NUS services, BLE bond/encryption inspection helpers, the clean phase-sounding `ble_channel_sounding` API, and the `Grtc` compatibility layer used by the new peripheral examples.
- The packaged generic `Bluetooth` wrapper now hardens mixed advertise/scan controller state, unregisters tracked services on `Bluetooth.end()`, and guards dynamic GATT allocation and read/write callback paths more defensively so BLE sketches fail cleanly instead of getting stuck in stale or partially-registered state.
- The packaged Zephyr base config now sets `CONFIG_BT_ID_MAX=2` for custom clean-core advertiser identities, `CONFIG_BT_BAS=y` so clean battery/GATT examples expose a real Battery Service, and `CONFIG_SETTINGS`/`CONFIG_BT_SETTINGS` plus larger Bluetooth stack sizes so bond persistence and pairing complete reliably on this core.
- Arduino GPIO on the packaged XIAO variant now uses raw active-low electrical semantics for `LED_BUILTIN` and `PIN_BUTTON`, matching the clean core: `LED_BUILTIN` is off at `HIGH` and on at `LOW`, while `PIN_BUTTON` reads `HIGH` released and `LOW` pressed.
- The bundled clean-derived example set is now compile-swept as a group: all 73 copied clean parity sketches build on `seeed:nrf54l15:xiao_nrf54l15`.
- Clean-core channel sounding examples are now bundled here as the phase-sounding `BleChannelSoundingInitiator` and `BleChannelSoundingReflector` sketches. The packaged Zephyr/NCS headers in this repo still expose a reduced AUXDATA register surface versus the clean core, so this parity path currently tracks the clean examples' `rtt=disabled_clean_core` mode rather than claiming raw RTT-ranging parity.
